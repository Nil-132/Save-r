...
async def _batch(event):
    if not event.is_private:
        return
    # Only do forcesub check if FORCESUB is actually set
    if fs:
    ...
    async with Drone.conversation(event.chat_id) as conv: 
        if fs:
            # The conv.send_message below is unconditionally sent, but we already checked fs above;
            # if fs was None, we skip the forcesub fully.
            pass
